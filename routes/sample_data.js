...
router.get("/", function(request, response, next){
	...
	db.serialize(function(){
		db.all(query, function(error, data){
			...
			{
				response.render('sample_data', {title:'留言評論', action:'list', sampleData:data});
			}
...
router.get("/add", function(request, response, next){
	response.render("sample_data", {title:'新增留言', action:'add'});
...
router.get('/edit/:id', function(request, response, next){
	...
	db.serialize(function(){
		db.all(query, function(error, data){
			...
			{
				response.render('sample_data', {title: '評論更改', action:'edit', sampleData:data[0]});
			}
